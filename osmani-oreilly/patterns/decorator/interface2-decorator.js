...
Interface.ensureImplements = function (object) {
    ...
    for (var i = 1, len = arguments.length; i < len; i++) {
        ...
        if (module.constructor !== Interface) {
            throw new Error('Function Interface.ensureImplements expects arguments' + 'two and above to be instances of Interface.');
        }
        for (var j = 0, methodsLen = module.methods.length; j < methodsLen; j++) {
            var method = module.methods[j];
            if (!object[method] || typeof object[method] !== 'function') {
                throw new Error('Function Interface.ensureImplements: object ' + 'does not implement the ' + module.name + ' module. Method ' + method + ' was not found.');
            }
        }
    }
};
